...
- New tool `hdf2neggia` which converts any HDF5 file compatible into something compatible with XDS+neggia plugin
...
- Fix regression related to the deprecation of `fopen` on MSVC compilers (validation code)
- Clean-up packaging
- Support Python 3.7-3.12
...
- Clean-up some debugging prints in CbfImage
...
- New support for HDF5 files generated by Bragg-sparsify (definitively read-only)
- Improved support for CBF: Magic bytes, headers, + other bug corrections
- Improved support for Tiff: Miss interpretation of colormap tags
- Improved support for GE files generated at APS
...
- Fix conversion from EDF to TIFF
