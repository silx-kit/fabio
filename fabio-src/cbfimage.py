...
#        backport contents of the CIF data to the headers
    ...
    def read(self, fname, frame=None):
        ...
        for i in  self.cif:
            print(i)
        if self.cif["_array_data.data"] == b"CIF Binary Section":
            self.cbs += infile.read(len(STARTER) + int(self.header["X-Binary-Size"]) - len(self.cbs) + self.start_binary)
        else:
            if len(self.cif["_array_data.data"]) > int(self.header["X-Binary-Size"]) + self.start_binary + len(STARTER):
                self.cbs = self.cif["_array_data.data"][:int(self.header["X-Binary-Size"]) + self.start_binary + len(STARTER)]
            else:
                self.cbs = self.cif["_array_data.data"]
        binary_data = self.cbs[self.start_binary + len(STARTER):]

        ref = self.header["Content-MD5"]
...
#            binary_blob += "\x00" * rem
        ...
        binary_block = [
                        ...
                        b"X-Binary-Size: %d" % (len(binary_blob)),
                        b"X-Binary-ID: 1",
                        b'X-Binary-Element-Type: "%s"' % (dtype),
                        b"X-Binary-Element-Byte-Order: LITTLE_ENDIAN" ,
                        b"Content-MD5: %s" % md5sum(binary_blob),
                        b"X-Binary-Number-of-Elements: %s" % (self.dim1 * self.dim2),
                        b"X-Binary-Size-Fastest-Dimension: %d" % self.dim1,
                        b"X-Binary-Size-Second-Dimension: %d" % self.dim2,
                        b"X-Binary-Size-Padding: %d" % 1,
                        b"",
                        STARTER + binary_blob,
                        b"",
                        b"--CIF-BINARY-FORMAT-SECTION----"
        ...
        self.cbf = b"\r\n".join(binary_block)
        block = b"\r\n".join([b"", b"_array_data.data", b";", self.cbf, b";"])
        self.cif.pop("_array_data.data", None)
    ...
    def _parseCIF(self, bytes_text):
        ...
        for i in range(len(fields) - 1):
            ...
            if fields[i][0] == self.UNDERSCORE and fields[i + 1][0] != self.UNDERSCORE:
                self[(fields[i]).decode("ASCII")] = fields[i + 1]
