...
import os, logging, struct, md5, base64
...
#                        long int 64 bits
    ...
    def md5sum(blob):
        ...
        """
        return base64.b64encode(md5.md5(blob).digest())
    ...
    def write(self, fname):
        ...
        @type: string
        """
        binary_blob = self.compress_ByteOffet(self.data)
        l = len(binary_blob)
        if (l % PADDING) != 0:
        ...
        binary_block = [
                        ...
                        "X-Binary-Element-Byte-Order: %s" % (self.endianness()),
                        "Content-MD5: %s" % self.md5sum(binary_blob[4:]),
                        ...
                        "X-Binary-Size-Padding: %d" % PADDING,
                        "",
                        binary_blob,
                        "--CIF-BINARY-FORMAT-SECTION----"
                        ]
...
#############################################################################################
    ...
    def saveCIF(self, _strFilename="test.cif", linesep=os.linesep):
